...
class LabjackU12Tests(unittest.TestCase):
    ...
    def test_basics(self):
        ...
        self.l.local_id()
        self.l.calibration()
        assert self.l.firmware_version() == 1.10
    ...
    def test_digital_out(self):
        ...
        assert (state_d & 0x55aa & 0x5a5a) == (0x55aa & 0x5a5a)
        assert (~state_d & 0x55aa & ~0x5a5a) == (0x55aa & ~0x5a5a)
        assert (state_io & 0x5 & 0x3) == (0x5 & 0x3)
        assert (~state_io & 0x5 & ~0x3) == (0x5 & ~0x3)
    ...
    def test_analog_in_out(self):
        ...
        for v in range(0, 6, 1):
            ...
            for i in r:
                assert abs(v-i) < .1, "measured %g for %g" % (i, v)
    ...
    def test_single_analog_in_cal_5v(self):
        ...
        """
        for s,c,e in [(5, 6, .1), (0, 7, .01), (2.5, 4, .01), (0, 5, .01)]:
            for v in self.l.input(channels=(c,c,c,c), gains=(1,1,1,1))[0]:
                assert abs(s-v) < e, "%g is not %g, channel %g" % (v, s, c)
    ...
    def test_diff_analog_in_cal_5v(self):
        ...
        for g in self.l.gains:
            for s,c,e in [(5, 11, .1), (2.5, 10, .05)]:
                ...
                if s*g > 20:
                    if s*g > 25:
                        assert v[3], "%s should be overvoltage (%g, %g)" % \
                            (v, s, g)
                    continue
                for i in r:
                    assert abs(s-i) < e,\
                            "%g is not %g, channel %g, gain %g" % \
                            (i,s,c,g)
    ...
    def test_burst(self):
        ...
        for v in self.l.burst_sync(channels=chans, gains=gains,
            ...
            for r in v[0]:
                assert abs(r-2.5) < .1, "%s should be cal, 2.5v" % v[0]
        scans_sec = 4*scans/(time.time()-a)
        assert scans_sec > 4000, "scan took too long (%g Hz)" % scans_sec
    ...
    def test_stream(self):
        ...
        for v in self.l.stream_sync(channels=chans, gains=gains, 
            ...
            for r in v[0]:
                assert abs(r-2.5) < .1, "%s should be cal, 2.5v" % v[0]
        scans_sec = 4*scans/(time.time()-a)
        assert scans_sec > 1200, "scan took too long (%g Hz)" % scans_sec

    def test_counter(self):
        self.l.count(reset=True)
        c,t = self.l.count(reset=False)
        assert c == 0, "counter is %g" % c
